...
std::string AcceptOrderHandler::HandleRequestThrow(
    ...
    if (producer_) {
        try {
            producer_->PublishOrderAccepted(*accepted, *driver_record->id);
            LOG_INFO() << "[match] Published OrderAccepted event for order_id=" << order_id;
        } catch (const std::exception& e) {
            LOG_WARNING() << "[match] Failed to publish OrderAccepted event: " << e.what();
        }
    }
