...
std::string CreateUserHandler::HandleRequestThrow(
    ...
    if (producer_) {
        try {
            producer_->PublishUserCreated(*user);
            LOG_INFO() << "[input] Published UserCreated event for user_id=" << *user->id;
        } catch (const std::exception& e) {
            LOG_WARNING() << "[input] Failed to publish UserCreated event: " << e.what();
        }
    }
...
std::string RegisterDriverHandler::HandleRequestThrow(
    ...
    if (producer_) {
        try {
            producer_->PublishDriverRegistered(*driver);
            LOG_INFO() << "[input] Published DriverRegistered event for driver_id=" << *driver->id;
        } catch (const std::exception& e) {
            LOG_WARNING() << "[input] Failed to publish DriverRegistered event: " << e.what();
        }
    }
