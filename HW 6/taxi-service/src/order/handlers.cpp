...
CreateOrderHandler::CreateOrderHandler(
    ...
    try {
        producer_ = &context.FindComponent<TaxiEventProducer>("taxi-event-producer");
    } catch (const std::exception&) {
        producer_ = nullptr;
    }
}
...
std::string CreateOrderHandler::HandleRequestThrow(
    ...
    if (producer_) {
        try {
            producer_->PublishOrderCreated(*order);
            LOG_INFO() << "[order] Published OrderCreated event for order_id=" << *order->id;
        } catch (const std::exception& e) {
            LOG_WARNING() << "[order] Failed to publish OrderCreated event: " << e.what();
        }
    }
...
std::string CompleteOrderHandler::HandleRequestThrow(
    ...
    if (producer_) {
        try {
            producer_->PublishOrderCompleted(*completed);
            LOG_INFO() << "[order] Published OrderCompleted event for order_id=" << order_id;
        } catch (const std::exception& e) {
            LOG_WARNING() << "[order] Failed to publish OrderCompleted event: " << e.what();
        }
    }
